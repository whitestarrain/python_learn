...
class TiebaSpider:
    def __init__(self, tieba_name):
        ...
        self.start_url = "http://tieba.baidu.com/mo/q----,sz@320_240-1-3---2/m?kw=" + \
            tieba_name+"&pn=0"
    ...
    def get_content_list(self, html_str):  # 提取数据
        ...
        for div in div_list:
            ...
            item["title"] = div.xpath(
                "./a/text()")[0] if len(div.xpath("./a/text()")) > 0 else None
            item["href"] = self.part_url + \
                div.xpath(
                    "./a/@href")[0] if len(div.xpath("./a/@href")) > 0 else None
            item["img_list"] = self.get_img_list(item["href"], [])
            item["img_list"] = [requests.utils.unquote(
                i).split("src=")[-1] for i in item["img_list"]]
